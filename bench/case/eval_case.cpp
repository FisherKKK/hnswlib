...
IndexPtr CreateIndex(const std::string &index_name, const std::string &create_params) {
    auto parsed_param = JsonType::parse(create_params);
    auto dim = parsed_param["dim"].get<int64_t>();
    std::string dtype = parsed_param["dtype"];
    std::string metric_type = parsed_param["metric_type"];

    auto index_param = parsed_param["index_param"];
    auto max_elements = index_param["max_elements"].get<int64_t>();
...
EvalCase::EvalCase(std::string dataset_path, std::string index_path, IndexPtr index)
...
EvalCase::MakeInstance(const EvalConfig& config, std::string type) {
    ...
    auto index = CreateIndex(index_name, create_params);
    ...
    if (type == "build") {
        return std::make_shared<BuildEvalCase>(dataset_path, index_path, index, config);
    }
    if (type == "search") {
        return std::make_shared<SearchEvalCase>(dataset_path, index_path, index, config);
    }
    return nullptr;
}
}  // namespace vsag::eval
