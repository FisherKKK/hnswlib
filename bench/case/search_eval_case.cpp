...
#pragma omp parallel for schedule(dynamic)
        for (int64_t id = 0; id < min_query; ++id) {
            ...
            auto result = this->index_->searchKnn(query_vector, topk);
            ...
            while (result.size() > 0)
            {
                neighbors.emplace_back(result.top().second);
                result.pop();
            }
            const int64_t* neighbors_ptr = neighbors.data();
            int64_t* ground_truth_neighbors = dataset_ptr_->GetNeighbors(i);
            auto record = std::make_tuple(
                neighbors_ptr, ground_truth_neighbors, dataset_ptr_.get(), query_vector, topk);
